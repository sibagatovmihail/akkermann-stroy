...
(function () {
  ...
  /* initGallerySlider() is deferred to loadCmsGalleryItems() so CMS images
     are already in the DOM when slides are built. */
  ...
  /* ==========================================
     CMS Gallery — append items from content/gallery.json
     ========================================== */
    ...
    fetch('content/gallery.json')
      ...
      .then(function (data) {
        ...
        items.forEach(function (item) {
          ...
          var label = CMS_LABELS[item.category] || item.category;
          var displaySrc = optimizeCloudinaryUrl(item.image);

          div.innerHTML =
            '<a href="' + item.image + '" class="glightbox" data-type="image">' +
            '<img src="' + displaySrc + '" alt="' + item.alt + '" loading="lazy"></a>' +
            '<div class="gallery__tag-cover"><span class="gallery__tag">' + label + '</span></div>';
